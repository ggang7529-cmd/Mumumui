...
export async function onRequestPost(context) {
  ...
  if (existing) {
    await env.DB.prepare("DELETE FROM comment_likes WHERE comment_id = ?1 AND user_id = ?2").bind(id, user.uid).run();
  } else {
    await env.DB.prepare("INSERT INTO comment_likes (comment_id, user_id) VALUES (?1, ?2)").bind(id, user.uid).run();
  }
